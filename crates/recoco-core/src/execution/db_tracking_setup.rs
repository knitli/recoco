...
/// Get the fully qualified tracking table name (schema.table)
async fn get_qualified_tracking_table_name_from_ctx(table_name: &str) -> Result<String> {
    let lib_context = get_lib_context().await?;
    Ok(format!(
        "\"{}\".\"{}\"",
        lib_context.internal_schema, table_name
    ))
}

/// Get the fully qualified tracking table name with explicit schema
fn get_qualified_tracking_table_name(schema: &str, table_name: &str) -> String {
...
async fn upgrade_tracking_table(
    ...
    if existing_version_id < 1 && desired_state.version_id >= 1 {
        // Ensure the internal schema exists before creating the table
        ensure_internal_schema_exists(pool).await?;

        let qualified_table_name =
            get_qualified_tracking_table_name_from_ctx(&desired_state.table_name).await?;
...
async fn create_source_state_table(pool: &PgPool, table_name: &str) -> Result<()> {
    ...
    let qualified_table_name = get_qualified_tracking_table_name_from_ctx(table_name).await?;
    let query = format!(
        "CREATE TABLE IF NOT EXISTS {qualified_table_name} (
            source_id INTEGER NOT NULL,
            key JSONB NOT NULL,
    ...
    pub async fn apply_change(&self) -> Result<()> {
        ...
        if let Some(desired) = &self.desired_state {
            for lagacy_name in self.legacy_tracking_table_names.iter() {
                let qualified_legacy = get_qualified_tracking_table_name_from_ctx(lagacy_name).await?;
                let query = format!(
                    "ALTER TABLE IF EXISTS {} RENAME TO {}",
                    qualified_legacy, desired.table_name
                );
                sqlx::query(&query).execute(pool).await?;
            }
            ...
            for lagacy_name in self.legacy_tracking_table_names.iter() {
                let qualified_legacy = get_qualified_tracking_table_name_from_ctx(lagacy_name).await?;
                let query = format!("DROP TABLE IF EXISTS {}", qualified_legacy);
                sqlx::query(&query).execute(pool).await?;
            }
        }
        ...
        if let Some(source_state_table_name) = source_state_table_name {
            for lagacy_name in self.legacy_source_state_table_names.iter() {
                ...
                let query = format!(
                    "ALTER TABLE IF EXISTS {} RENAME TO {}",
                    qualified_legacy, source_state_table_name
                );
                sqlx::query(&query).execute(pool).await?;
            }
            if !self.source_state_table_always_exists {
